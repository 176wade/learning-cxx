...
int main(int argc, char **argv) {
    ...
    A ab = B(5); // 然而这个代码可以编译和运行！
                 // 观察打印出的信息，推测把大象放进冰箱分几步？
                 // 这样的代码是“安全”的吗？

    // 解释：
    // 当创建 B(5) 时，会先调用 A 的构造函数初始化基类部分，再调用 X 的构造函数初始化成员变量 x，
    // 最后调用 B 的构造函数本身。接着，A ab = B(5) 实际上会调用 A 的拷贝构造函数，只复制 B 的 A 部分。
    // 这种操作被称为“切片”，即 B 对象中除 A 以外的部分被“切掉”了。这可能会导致逻辑上的错误，尤其是在 B 中有额外资源管理的情况下。
